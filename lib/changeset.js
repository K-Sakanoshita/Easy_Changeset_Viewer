	...
	readChangeset() {
		...
		function xhr_get(changesets, edtime, resolve, reject) {
				...
				xhr.onreadystatechange = function () {
					...
					if (xhr.status >= 200 && xhr.status < 400) {
						...
						if (newchanges.length >= 100) {
							let lastCreatedAt = new Date(newchanges[newchanges.length - 1].attributes.created_at.nodeValue);
							let nextEdtime = lastCreatedAt.toISOString().slice(0, 19) + "Z";
							if (nextEdtime == edtime) {
								lastCreatedAt.setUTCSeconds(lastCreatedAt.getUTCSeconds() - 1);
								nextEdtime = lastCreatedAt.toISOString().slice(0, 19) + "Z";
							}
							xhr_get(changesets, nextEdtime, resolve, reject);
						} else {
			...
			function calcStartEndTime(edtime0) {		// changesetの取得期間を計算
				...
				let edtime3 = edtime0 == ""
					? basic.formatDate(edtime2, "YYYY-MM-DDThh:mm:ssZ")
					: edtime2.toISOString().slice(0, 19) + "Z";

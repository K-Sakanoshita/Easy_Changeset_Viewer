    ...
    */
    async getFullDataFromChangeset(changesetId) {
        ...
        if (nodeIds.length === 0 && wayIds.length === 0 && relationIds.length === 0) {
            throw new Error('This changeset does not contain any OSM objects.');
        }
    ...
    */
    async getFullDataFromUserName(params) {
        ...
        const convertToUTC = (isoDateStr) => {
            const normalized = isoDateStr.replace(
                /^(\d{4})\/(\d{2})\/(\d{2}) (\d{2}:\d{2})([+-]\d{2}:\d{2})$/,
                '$1-$2-$3T$4:00$5'
            );
            const date = new Date(normalized); // タイムゾーン付きの文字列なら正確にパースされる
    ...
    async fetchOverpassData(query) {
        ...
        if (data.remark !== undefined && /timeout|timed out|time out/i.test(data.remark)) {
            throw new Error(`Overpass API timeout: ${data.remark}`);
        }
        return data;
    }

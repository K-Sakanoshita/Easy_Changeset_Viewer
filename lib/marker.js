class Marker {
    ...
    makeMarker(element, mapper) {		// 指定したelementからマーカーを作成
        ...
        let escapedUsername = basic.htmlspecialchars(username);
        ...
        contents += `<button onclick="easycs.viewDetail({username:decodeURIComponent('${encodedUsername}'),stdate:'${start_datetime.value}',eddate:'${end_datetime.value}',tz:'${timezones.value}'})">all detail</button>`;
        contents += `<button onclick="easycs.toggleMapper(${rank})">delete</button></div>`;
        contents += `User Name: <a href="https://osm.org/user/${encodedUsername}">${escapedUsername}</a><br>`;
        ...
        contents += `<span id="detailSpinner" class="detail-spinner" role="status" aria-label="Loading details" hidden></span>`;
        let marker;

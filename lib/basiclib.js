...
class Basic {
    copyLink() {	// URL共有機能
        params.set("sttime", this.parseDateTime(start_datetime.value).getTime() / 1000);
        params.set("edtime", this.parseDateTime(end_datetime.value).getTime() / 1000);
        params.set("timezn", timezones.value);
        let url = location.origin + location.pathname + "?" + params.toString() + location.hash;
